...
*   **路由**: `/admin`
*   **功能**:
    *   账号登录：扫码登录哔哩哔哩账号，获取用户cookie(sessdata)。
    *   连接管理：输入 Room ID和sessdata，确认房间连接状态。
    *   设置：
        *   退出登录：重置右上角登录状态，清空sessdata输入框。
        *   移除用户：删除数据库中指定用户记录
        *   清理缓存：清理数据库中历史弹幕、礼物
        *   配置动画：设置触发不同动画的门槛(监听到不同礼物、舰队类型时对应不同的动画)
        *   恢复默认设置：重置所有数据和设置
    *   美术配置：上传动画文件。
    *   预览窗口：实时预览 OBS 输出效果。
*   **UI**: 
    *   使用浏览器访问。
    *   标准 Ant Design 后台风格，清晰、整洁。
    *   使用SVG图标，禁止使用emoji。
    *   界面布局说明：
        *   整体使用仪表盘风格布局。分为顶部导航区域、连接管理区域、美术配置区域、弹幕预览区域、sc预览区域、礼物(上舰)预览区域、底部动画预览窗口区域。
        *   顶部导航区域：
            *   通常状态：左侧显示系统标题、连接状态(房间号、房间标题)，右侧显示登录状态(用户名，鼠标悬停时展开二级菜单)和设置按钮。
            *   初始化：连接状态显示“未连接”，房间标题显示为“-”。用户名显示“未登录”，鼠标悬停时展开二级菜单显示登录按钮。
        *   控制台分页：
            *   连接管理区域
            *   通常状态：显示当前连接的房间号和房间标题。
            *   初始化：显示“未连接”。
        *   登录按钮：右上角位置，点击后模拟浏览器访问哔哩哔哩扫码登录页面，在登录按钮下方显示登录二维码，等待用户扫码登录。登录成功后自动获取用户cookie(sessdata)并填入sessdata输入框，登录按钮文字替换为登出，按钮左侧显示用户昵称。
        *   连接管理输入框：输入 Room ID和sessdata。
        *   连接按钮：点击后连接到指定房间，获取房间可用礼物信息，并通知后端开始监听弹幕、礼物等事件。
        *   设置按钮：左上角位置，点击后展开设置弹窗。
            *   退出登录：重置右上角登录状态，清空sessdata输入框。
            *   移除用户：删除数据库中指定用户记录
            *   清理缓存：清理数据库中历史弹幕、礼物
            *   配置动画：设置触发不同动画的门槛(监听到不同礼物、舰队类型时对应不同的动画)
...
*   **系统使用流程说明**:
    *   1.点击后模拟浏览器访问哔哩哔哩扫码登录页面，弹窗显示登录二维码，等待用户扫码登录，点击空白处关闭弹窗。登录成功后关闭二维码弹窗并提示用户登录成功，用户名显示为用户昵称，登录按钮文字替换为登出，按钮左侧显示用户昵称。
...
---
**附注**: 
*   **性能优化**: 同屏角色不超过 10 个，移除更早的角色，确保新角色的加入和动画播放。
